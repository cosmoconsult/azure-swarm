...
resource "azurerm_network_interface" "leader" {
  name                = "${var.name}-leader-nic"
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_windows_virtual_machine" "leader" {
  name                            = "${var.name}-leader-vm"
  ...
  network_interface_ids = [
    azurerm_network_interface.leader.id,
  ]
...
resource "azurerm_virtual_machine_extension" "initLeader" {
  name                       = "initLeader"
  virtual_machine_id         = azurerm_windows_virtual_machine.leader.id
  ...
  protected_settings = <<PROTECTED_SETTINGS
    {
      "commandToExecute": "powershell -ExecutionPolicy Unrestricted -File mgrInitSwarmAndSetupTasks.ps1 -externaldns \"${var.name}.${var.location}.cloudapp.azure.com\" -email \"${var.eMail}\" -branch \"${var.branch}\" -additionalScript \"${var.additionalScriptJumpbox}\" -name \"${var.name}\" -isLeader"
    }
  PROTECTED_SETTINGS
...
resource "azurerm_virtual_machine_extension" "initMgr" {
  ...
  auto_upgrade_minor_version = true
  depends_on                 = [
    azurerm_virtual_machine_extension.initLeader
  ]
...
resource "azurerm_key_vault_access_policy" "leader" {
  key_vault_id = azurerm_key_vault.main.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = azurerm_windows_virtual_machine.leader.identity.0.principal_id
