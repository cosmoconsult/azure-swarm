...
$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Unrestricted -Command `"& 'c:\iac\workerConfig.ps1' -name $name -images '$images' -additionalScript '$additionalScript' -branch '$branch'`" 2>&1 >> c:\iac\log.txt"
...
$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Unrestricted -Command `"& 'c:\iac\workerConfig.ps1' -name $name -images '$images' -additionalScript '$additionalScript' -branch '$branch' -restart`" 2>&1 >> c:\iac\log.txt"
