...
if ($isFirstMgr) {
    ...
    while ($tries -le 10) { 
        try {
            ...
            $result = Invoke-WebRequest -Uri https://$name-vault.vault.azure.net/secrets/JoinCommand?api-version=2016-10-01 -Method PUT -Headers @{Authorization = "Bearer $KeyVaultToken" } -Body (ConvertTo-Json $Body) -ContentType "application/json" -UseBasicParsing -RetryIntervalSec 10 -MaximumRetryCount 5 
            Write-Debug $result
            ...
            $result = Invoke-WebRequest -Uri https://$name-vault.vault.azure.net/secrets/JoinCommandMgr?api-version=2016-10-01 -Method PUT -Headers @{Authorization = "Bearer $KeyVaultToken" } -Body (ConvertTo-Json $Body) -ContentType "application/json" -UseBasicParsing -RetryIntervalSec 10 -MaximumRetryCount 5 
            Write-Debug $result
            ...
            $secretJsonMgr = (Invoke-WebRequest -Uri https://$name-vault.vault.azure.net/secrets/JoinCommandMgr?api-version=2016-10-01 -Method GET -Headers @{Authorization = "Bearer $KeyVaultToken" } -UseBasicParsing -RetryIntervalSec 10 -MaximumRetryCount 5 ).content | ConvertFrom-Json
    ...
    while ($tries -le 10) { 
        try {
            ...
            $content = $response.Content | ConvertFrom-Json
            $KeyVaultToken = $content.access_token
            $secretJson = (Invoke-WebRequest -Uri https://$name-vault.vault.azure.net/secrets/JoinCommandMgr?api-version=2016-10-01 -Method GET -Headers @{Authorization = "Bearer $KeyVaultToken" } -UseBasicParsing -RetryIntervalSec 10 -MaximumRetryCount 5 ).content | ConvertFrom-Json
...
Invoke-WebRequest -UseBasicParsing -Uri "https://raw.githubusercontent.com/cosmoconsult/azure-swarm/$branch/scripts/profile.ps1" -OutFile $PROFILE.AllUsersAllHosts -RetryIntervalSec 10 -MaximumRetryCount 5 

# Setup tasks
Write-Debug "Download task files"
Invoke-WebRequest -UseBasicParsing -Uri "https://raw.githubusercontent.com/cosmoconsult/azure-swarm/$branch/scripts/mgrConfig.ps1" -OutFile c:\scripts\mgrConfig.ps1 -RetryIntervalSec 10 -MaximumRetryCount 5 
